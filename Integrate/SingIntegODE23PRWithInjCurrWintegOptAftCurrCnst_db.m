  ...
  function [SingPRWithInjCurrbefore, SingPRWithInjCurrafter]= SingIntegODE23PRWithInjCurrWintegOptAftCurrCnst_db(aPR,uAmpsPermsecCm2,delay,Tend,VdsOut,VsThresh,SomaInj,termaftevent,varargin,tafter)
...
if nargin == 0 || isempty(nargin)
    options = odeset('RelTol',1e-4,'Stats','off','MaxStep',1e-1,'Refine',4,'Events',@events,...
                 'OutputSel',1);
elseif  nargin == 1
    options = odeset('RelTol',varargin(1,1),'Stats','off','MaxStep',1e-1,'Refine',4,'Events',@events,...
                 'OutputSel',1);
elseif nargin == 2
    options = odeset('RelTol',varargin(1,1),'AbsTol',varargin(1,2),'Stats','off','MaxStep',1e-1,'Refine',4,'Events',@events,...
                 'OutputSel',1);
elseif nargin == 3
    options = odeset('RelTol',varargin(1,1),'AbsTol',varargin(1,2),'Stats','off','MaxStep',varargin(1,3),'Refine',4,'Events',@events,...
                 'OutputSel',1);
end 
...
SingPRWithInjCurrbefore.te = te;
SingPRWithInjCurrbefore.ye = ye;
SingPRWithInjCurrbefore.ie = ie;

SingPRWithInjCurrbefore.YMultcol=YMultcol;
SingPRWithInjCurrbefore.T = T;


SingPRWithInjCurrbefore.etime=toc(tstart1);
SingPRWithInjCurrbefore.datetime=datestr(now);
SingPRWithInjCurrbefore.file=mfilename;


SingPRWithInjCurrbefore.PR=aPR;
SingPRWithInjCurrbefore.NumN=1;
SingPRWithInjCurrbefore.uAmpsPerCm2=uAmpsPermsecCm2;
SingPRWithInjCurrbefore.VdsOut=VdsOut;
SingPRWithInjCurrbefore.Tend=Tend;
SingPRWithInjCurrbefore.delay=delay;
if isempty(te) 
    SingPRWithInjCurrbefore.idxteVs=size(T,1);
else
    SingPRWithInjCurrbefore.idxteVs=find(T>=te(1,1),1,'First');
end
...
SingPRWithInjCurrbefore.file=mfilename;
termaftevent=false;
[T2,YMultcol2,y2te,y2e,y2ie] = ode23(@(t,Y) PR94NoSynAft(t,Y),[te, (te+tafter)],ye,options);

SingPRWithInjCurrafter.te = y2te;
SingPRWithInjCurrafter.ye = y2e;
SingPRWithInjCurrafter.ie = y2ie;

SingPRWithInjCurrafter.YMultcol=YMultcol2;
SingPRWithInjCurrafter.T = T2;


SingPRWithInjCurrafter.etime=toc(tstart1);
SingPRWithInjCurrafter.datetime=datestr(now);
SingPRWithInjCurrafter.file=mfilename;


SingPRWithInjCurrafter.PR=aPR;
SingPRWithInjCurrafter.NumN=1;
SingPRWithInjCurrafter.uAmpsPerCm2=uAmpsPermsecCm2;
SingPRWithInjCurrafter.VdsOut=VdsOut;
SingPRWithInjCurrafter.Tend=Tend;
SingPRWithInjCurrafter.delay=delay;
if isempty(te) 
    SingPRWithInjCurrafter.idxteVs=size(T,1);
else
    SingPRWithInjCurrafter.idxteVs=find(T>=te(1,1),1,'First');
end
...
function dY = PR94NoSynAft(t,Y)

   Nn=sqrt(size(Y,1));
   Y = reshape(Y,1,8);
    Isinj=SingPRWithInjCurrbefore.te*uAmpsPermsecCm2+aPR.Isinj;
    aPR.Isinjaft=Isinj;
    Cm=aPR.Cm;
    gL=aPR.gL;
    gNa=aPR.gNa;
